...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
  ...
  private String[] storageTypeToString(StorageType[] storageTypes) {
    List<String> storageTypesString = new ArrayList<String>();
    for (StorageType storageType : storageTypes) {
      LOG.info("Locality test: " + storageType.toString());
      storageTypesString.add(storageType.toString());
    }
    return storageTypesString.toArray(new String[0]);
  }
  ...
  private StorageType[] stringToStorageType(String[] storageTypesString) {
    ...
    for (String storageTypeString : storageTypesString) {
      storageTypes.add(StorageType.valueOf(storageTypeString));
    }
  ...
  throws IOException {
    ...
    LOG.info("Locality test: bytesInThisBlock = " + bytesInThisBlock + ", splitSize = " + splitSize);
    if (bytesInThisBlock >= splitSize) {
      String[] storageTypesString = storageTypeToString(blkLocations[startIndex].getStorageTypes());
      LOG.info("Locality test: storage type length = " + storageTypesString.length);
